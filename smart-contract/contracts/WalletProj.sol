...
contract WalletProj is ERC721 {
    ...
    modifier contractHasValidBalance(uint256 _transferAmount) {
        require(
            ...
            "Contract must have a balance greater than or equal to the amount being transferred"
        );
        _;
    }
    ...
    function transferAmountFromContract(
        uint _transferAmount,
        address _someone
    ) public onlyOwner contractHasValidBalance(_transferAmount) {
        payable(_someone).transfer(_transferAmount);
    }

    function addAddressToWhitelist(
        address _address
    ) public onlyOwner addressIsNotWhiteListed(_address) {
